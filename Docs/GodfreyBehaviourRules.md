...
- **Brain:** do not emit `[gesture:GreetingWelcome_01]` / `_02` / `_03` (presence owns Welcome).

### R3 — Listening only while the visitor is speaking
- **Status:** implemented
- **Rule:** Listening-pool AS (`ListeningAttentive` / `Curious` / `Concerned` / `Nodding`) play **only while Godfrey is actually listening** (Speak-green / visitor speech). When the lantern goes **Wait** (visitor turn finished, Brain generating, `BeginThinking` / `NotifyReplyIncoming`), play a **thinking-pool** hold until he is ready to speak: `ThinkingHandToChin`, `Thinking_01`/`_02`, `ThinkingDeepBreath_*`, `ThinkingRemembering`. `ThinkingScratchingHead` and `ThinkingCoy` are **rare** (about one thinking deck in eight). Do **not** use `ThinkingLookingToSea` / `LookingAway` (R7 / camera).
- **Verify:** `[Acting] play … context=Listening` only while Speak is green. After AskGodfrey / red Wait: `behaviour Thinking` then `thinking pool next`. No `AwaitBrainListening` / listening-pool lines during Brain wait.
...
- **Rule:** In-dialog idle holds play as **one-shots**, advancing through a **shuffled deck**. **Awaiting visitor** (Speak green) uses the listening pool. **Awaiting Brain** (Wait lantern) uses the thinking pool (R3). Soft blend AS→AS (`GodfreyDialogIdleMontageBlendIn/OutSeconds`, default **1.5 / 1.6**). Do **not** section-loop a single clip. **Early-chain** the next AS before blend-out so DefaultSlot weight never drops to MetaHuman RefPose (A-pose).
- **Verify:** `[Acting] play` after EndSpeaking (visitor’s turn) cycles `Listening*_01`; after visitor speech + red lantern, cycles `Thinking*`. Logs show `one-shot hold … will chain` / `reshuffled listening-pool deck` or `reshuffled thinking-pool deck`, not `section … set to loop`.
...
### R23 — Coat yields to the arms (cloth), arms keep authored pose
- **Status:** implemented
- **Problem:** Coat-clearance IK pushed hands/elbows off the jacket so they no longer punched through, but the arms read as held far from the body.
- **Rule:** Do **not** IK the arms away from the coat (`bGodfreyCoatClearance=false`). The assembled costume (`MHC_*_Outfits1`) is a leader-pose mesh with **no** clothing assets — enable Chaos Cloth from `OA_Casual_formal` / `CA_a`, bind it to Body, collide with the body physics asset (arm/hand capsules), and hide the rigid outfit mesh. Hands stay on the performance pose; the cloth dents in around them. Sea-idle pool still must not include `HandsClasped`. If sleeves lag the arms, lower `GodfreyCoatClothMaxDistanceScale` (or paint lower max-distance on the sleeve region of the cloth asset).
- **Verify:** PIE log `coat cloth sim on` / `spawned ChaosCloth` / `collision source=Body` — not `no cloth components found`. Describing / waist-front poses keep natural arm line; coat body/hem compresses instead of hands punching through. Tune under Project Settings → Animation|Costume.
...
- **Rule:** Brain **choreographs** speaking body. It must emit `[gesture:…]` at the start of every spoken reply and enough **distinct** CatalogIds to cover the whole line (takes last ~10–18s; a 40s wreck/court answer needs 3–5 different ids, never the same id twice). Unreal plays each Brain take **once, to completion**. Later `[gesture:]` markers that arrive while a take is still playing are **queued** (LLM tokens land early, not at the spoken beat). Cues during Engaging (Welcome prefetch) are stashed and applied at audible speak-start. If Brain omitted a gesture, or a take ends while he is still talking with no queued `[gesture:]`, Unreal fills with a **different** basic explaining clip (`Explaining_01`/`_02`/`Firmly`, Calm, Gentle, DescribeSequence) — never Greeting, Thinking, Idle, and never a repeat of the last stem. `[amused]`/`[emphasis]`/`[serious]` are mood only. Listening / thinking / SeaIdle pools stay Unreal-owned. `SummingUpHisCase_01` is a plea to be understood — cue it only when he asks them to judge him fairly.
- **Named take owns the slot:** Brain `[gesture:…]` arms a hold as soon as it plays, **even if audio has not started yet**. `BeginSpeaking` must **not** start the speaking pool over that overlay. **Do not** hold Welcome / Greeting / Listening / Idle — presence Welcome is interrupted when audible speech starts (short blend) so baked visemes cannot fight ACE. Pause / gaze / expression / `[amused]` / `[emphasis]` / `[serious]` are mood only and must **not** leave the Speaking state or start R16 Listening* while ACE is still playing. After a Brain take ends, play the **next queued** Brain `[gesture:]` if one arrived early, otherwise a **different** basic pool clip — never replay the same CatalogId in that utterance, and never start two pool clips on the same end (stale montage-ended / tick maintain). After audio ends, R16 1s hold then Listening* (do not replay Greeting*).
- **Verify:** `[Acting] play … context=NamedAction` matches Brain `[gesture:]`. Welcome prefetch: `stashed Brain speaking take` then `applying stashed Brain speaking take` at BeginSpeaking. A second `[gesture:]` while the first take is live logs `queued Brain speaking take` — **not** a second NamedAction play 0.3–2s later. After the first take ends: `queued` take plays, or `speaking pool next` with a **new** stem — **not** `waiting for Brain`, **not** `sustaining Brain speaking take`, **not** Greeting/Thinking while `bIsSpeaking`. Same-tick double pool plays log `ended after already advancing`. A second `[gesture:Explaining_01]` in the same utterance logs `skip repeating speaking take`. `[amused]`/`[emphasis]`/`[serious]` log `mood-only (stay Speaking)` or `reason=brain-owns-speaking-body`, **not** `performance state 3 -> 4`, **not** `[PostSpeech]` / `ListeningNodding` mid-line. Post-speech `[Acting] play` is `Listening*` only after ACE audio end. `Concerned_01` must play as Concerned, not remap to CalmExplanation.

### R15 — First dialog hold uses Greeting* once
- **Status:** implemented
- **Rule:** When Godfrey first transitions SeaIdle → in-dialog, the **first** await/listen body hold picks from `DialogGreetingPool`. Prefer `GreetingWelcome_03` (weighted) over `GreetingWelcome_01` / `GreetingNod` / `GreetingSmallSmile` / `GreetingHaveASeat`. `GreetingWelcome_02` is shelved (A-frame arms). Presence Welcome (R17) prefers `_03` then `_01`. **Playing presence Welcome consumes this once-per-encounter greeting** — post-speech / await holds after that use the listening pool (R9), not a second `GreetingWelcome`. Reset on return to SeaIdle.
- **Exclude:** `GreetingTurnToVisitor` (camera already frontal — see `bSkipEngageTurnMontage`).
- **Verify:** first `[Acting] play` after engage shows a `Greeting*` sequence; later holds (including `PostSpeechAwaitReply`) show `Listening*`. Do not replace Greeting mid-clip with ConversingIdle Listening (hold flag) **until speech starts**. Baked viseme/jaw curves on `AS_GreetingWelcome_03` (and other solved takes) are zeroed on the body overlay so they do not look like speech; ACE owns the mouth while he is speaking. Smile/dimple/corner-pull expression curves are kept.
...
- **Also:** mid-utterance speaking-pool advances use early soft chain (timeline/rate wall) so short clips do not hard-cut. Chain lead and blend-in/out are the same dialog times so the next take does not win in 0.45s. Do **not** early-chain over a story named take. Do **not** replay Greeting* after speech (R15 consumed by presence Welcome).
- **Verify:** logs show `[PostSpeech]: holding speaking body` then later `soft-blend speaking body -> Listening*`; Listening* `[Acting] play` is ~1s+ after `Speech Finished`. `[PostSpeech]` must **not** fire on `[emphasis]` / `[amused]` / `[serious]` while ACE is still playing.

### R4 — Listening ends when Godfrey speaks
- **Status:** implemented (`HandleSpeakingStarted` starts SpeakingIdle unless a Brain named take already owns UpperBody)
- **Rule:** As soon as Godfrey audible speech starts (`OnAnimationStarted` → BeginSpeaking), Listening / Welcome / Thinking overlay must stop (short blend). If a Brain `[gesture:…]` is already playing or stashed, that take **is** the speaking sequence. Otherwise Unreal starts a basic explaining pool clip so he is never idle while talking. Welcome / Greeting overlays are not story takes. **Do not wait for the EngageGreet timer** — if he is still Engaging when audio starts, finish engage and EnterSpeaking immediately.
...
- **Idle rule while in dialog:** after the optional first Greeting* hold (R15), **listening-pool** AS while Speak is green; **thinking-pool** AS while the lantern is Wait / Brain is generating (R3). Do **not** select Idle* overlays (`IdleStanding`, `IdleWeightShift`, `IdleRockingOnFeet`, `IdleLookingToSea_*`, SeaIdle exhibition hold) as the conversation hold. Planted `IdleStanding` on DefaultSlot is the stance only.
...
- **Status:** implemented
- **Rule:** Speaking body AS must stop when ACE audible playback ends. Do not mute ACE or `Stop()` the curve source on the normal finish path. `EndAudioSamples` runs **off the game thread at HTTP drain** so the last word is not starved of A2F curves and the mixer is not stalled. Face rest (`RequestRestPose`) latches after last voiced audio has drained through ACE's delay (LastVoice + BufferLength/DAC), using a capped RMS floor so quiet last syllables still count. Do not wait for sent-PCM hush (that left A2F chewing) and do not rest at LastVoice+0.10s (that froze the mouth while speech continued). Watchdog still waits until Wall has passed sent PCM plus BufferLength before EndSpeaking. Abort is the only mute/Stop path.
...
- **Next visitor (webcam still Present):** spoken goodbye does not wait for Empty. After the farewell wave returns to SeaIdle, Unreal waits **`PostFarewellSuccessorSeconds` (default 3s)** for them to leave. If occupancy is still Present, or if anyone speaks during that wait, play the arrival briefing card then Welcome (R17) as a **new** visit. STT is held until that Welcome. If they walk out within 3s → Empty + recapture as usual (no second Welcome). R10 unanswered-leave still recaptures empty (do not Welcome a vacant ghost).
...
- **Verify:** say "I've got to go now, goodbye" on the Speak-green mic → Brain log `visitor farewell`; Unreal `conversation end latched (brain conversationEnd (visitor_phrase))` then `BeginFarewell` after `[Speech] Speech Finished`. He must not continue the story or fire a dialog engagement prompt. Stay in frame after the wave → log `spoken goodbye — waiting 3.0s` then `post-goodbye new visitor (still-occupied)` + arrival card + Welcome. Speak during that wait → `post-goodbye new visitor (speech)` and the card, not a continuation of the last chat. Walk out within 3s → `post-goodbye successor cancelled (left)` / `zone-empty`, no Welcome. `"It's good to see you back, Godfrey."` and `"I want to see you clap your hands."` must **not** latch farewell (`see you` greeting / request, not a leave). `node scripts/check-conversation-end.js`.
...
- **Engage:** when `Present`, play the **arrival briefing card** and start Welcome together — **even if the mic already heard something**. STT must not `AskGodfrey` until that Welcome has been delivered this occupancy (Empty / Approaching / card-on-screen). **While the card is up**, Godfrey is already in-dialog on Greeting* (he is visible through the veil). Brain/TTS/A2F **prefetch** the spoken welcome with ACE Play held. When the card finishes, Unreal **releases the audible hold** so he speaks as soon as ACE has samples, and BeginSpeaking **interrupts Greeting** into the speaking pool (R4). Then `NotifyVisitorEngaged()` (R2) is already done. Speech-only exhibits (webcam off / no card) still arm Welcome then `AskGodfrey(WelcomeSpeakPrompt)` (~0.35s delay so greet starts first). The Welcome line must **end with a short question** (who they are, or whether they have been to sea).
- **Arrival briefing:** Constantia card + dim veil (`UGodfreyVisitorBriefingComponent`, auto-spawned with webcam presence). Copy: the way is far; GREEN = only one may speak, clearly into the microphone; RED = he cannot hear you; the way is not swift. Mic is held for the whole card (lantern stays Wait). Overlay Z is below the brass lantern so Speak/Wait remains visible. Fade in 0.8s, hold 12s, fade out 1.8s. Greeting* plays **during** the card; spoken welcome is prefetched (`AskGodfreyHeldAudible`) and becomes audible when the card ends (`released welcome audible hold`). Empty during the card cancels it (abort prefetch, SeaIdle, no Welcome). Next visitor after Empty sees it again. Operator **F8** skips. Project Settings → Vision|Presence (`bGodfreyEnableVisitorBriefing`). Godfrey does not speak this copy.
- **Leave:** freeze visitor pixels while Present. Occupancy uses **hysteresis**: enter at **0.06**, leave only below **0.03**, plus **leave dwell default 2s** of continuous emptiness, then `Present → Leaving → Empty` while in dialog → `RequestConversationEnd(webcam visitor left)` + goodbye `AskGodfrey(FarewellSpeakPrompt)` (name from Brain visitor profile when known; prompt asks for `[farewell]`). Wave runs after the line via R12 — do **not** `BeginFarewell` mid-sentence. Then recapture empty, but **skip recapture** if occupancy is still above the leave threshold (do not bake a standing visitor into the empty model). A **spoken** goodbye while occupancy never reaches Empty is R12 successor: 3s to leave after SeaIdle, then arrival card + Welcome if still occupied (or on speech).
...
- **Debug:** runtime perf HUD on by default (green top line; **F8** toggles it — F8 also skips the arrival briefing). Top-left box shows the current body `AS_*` name (**F6** toggles). Webcam preview off by default (F9 toggles); force occupied (F10); recapture empty (F11); 5s occupancy heartbeat while Present/Leaving (occ/mot/emptyAge). Top-right brass **signal lantern** Speak/Wait cue (Constantia labels) synced to mic accept window. Mic stays paused for the full ACE audible utterance (not only HTTP `IsStreaming`); Speak opens **~1.25s** after playback complete. Project Settings → Vision|Presence / Listen Cue. Track/format enumeration logs a one-line summary; per-format dump is `LogGodfreyVision` Verbose only (WMF webcams can expose 200+ formats).
- **Verify:** walk in (mic quiet or not) → arrival briefing card (log `arrival-card Welcome — Greeting now, prefetch speak`) while Greeting* plays; then `arrival briefing finished` + `released welcome audible hold` and he speaks with speaking-pool body, not a leftover Welcome. A stray STT word before Welcome must log `holding STT until arrival briefing/Welcome` and not skip the card. Walk out and stay out ~2s → `Present → Leaving → Empty`, `absence farewell latched`, `farewell speak AskGodfrey ok=1`, then goodbye + farewell wave, then empty recapture (`leave`). A brief occupancy dip while still standing must **not** farewell. Move a chair while empty → Approaching without Welcome, then `static-occupancy` or `stillness` recapture. F11 recaptures immediately. After he finishes a line, a lone STT `Cool.` / `Hello.` must log `dropping STT noise-hallucination` and not `AskGodfrey`. Talk over him while Wait → lantern reminder `He cannot hear you. Speak only when the lantern is green.` (log `speak-while-Wait reminder`); STT must stay paused.

### R24 — Speak-while-Wait reminder
- **Status:** implemented
- **Problem:** Visitors talk over Godfrey while the lantern is red; STT is paused so nothing is heard, and they do not know why.
- **Rule:** While the lantern is Wait because Godfrey is speaking (not the 1.25s echo guard, not the arrival card), keep capturing the mic for **local loudness only**. If visitor-level energy stays above speaker bleed for ~0.4s, show a Constantia reminder under the lantern for ~5s. Do **not** unpause STT or AskGodfrey. Cooldown ~10s. Tune under Project Settings → Vision|Listen Cue.
- **Verify:** talk into the HyperX while he is mid-reply → reminder appears, log `speak-while-Wait reminder`; his line is not cut and no visitor transcript is sent. Stay quiet → no reminder from his own voice in the room.
