...
- **Rule:** While speaking, play body AS from `SpeakingIdlePool` as **one-shots** in a shuffled deck (soft blend AS→AS). Newer takes (`Explaining_01/02`, `ExplainingFirmly_01`, `DescribingWhere_01`, `WantingToBeUnderstood_01`, `SummingUpHisCase_01`) are weighted ~3× so they play more often than older CalmExplanation / Describing* clips. `WantingYouToHear_01` is shelved (A-frame arms in the take). Never section-loop one speaking clip. Conversation overlays use the **dialog** blend times (`GodfreyDialogIdleMontageBlendIn/OutSeconds`, default **1.5 / 1.6**). `SummingUpHisCase_01` is a plea to be understood, not a courtroom summation — Brain should also cue it by meaning.
- **Verify:** `[Acting] play … context=SpeakingIdle` cycles `Explaining*` / `Wanting*` / `SummingUpHisCase` as well as older Describing/Speaking stems; logs show `speaking pool next` / `reshuffled speaking-pool deck` with the new stems repeated. `[SeqStart] … slot=UpperBody … blendIn=1.50 blendOut=1.60` on speaking/listening/gesture overlays.

### R15 — First dialog hold uses Greeting* once
- **Status:** implemented
- **Rule:** When Godfrey first transitions SeaIdle → in-dialog, the **first** await/listen body hold picks from `DialogGreetingPool`. Prefer `GreetingWelcome_03` (weighted) over `GreetingWelcome_01` / `GreetingNod` / `GreetingSmallSmile` / `GreetingHaveASeat`. `GreetingWelcome_02` is shelved (A-frame arms). Presence Welcome (R17) prefers `_03` then `_01`. Subsequent dialog holds use the listening pool (R9). Reset on return to SeaIdle.
...
- **Status:** implemented
- **Rule:** Speaking body AS must stop when ACE audible playback ends. Do not mute ACE, rest the face, or call `EndAudioSamples` until the playhead has passed sent PCM **plus ACE BufferLength (~0.35s)** — the device still holds that tail, so flushing at Wall≈Sent cut the Welcome closing question. After HTTP has drained, do not treat playhead-caught-curves as an ingest stall.
