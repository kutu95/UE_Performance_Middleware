...
void UGodfreyPerformerAnimationBridgeComponent::EnsureBodyMontagePlaybackReady()
{
	...
	// MetaHuman garment meshes leader-pose from Body — keep cinematic post-process enabled.
	...
	const float PlayRate, const bool bRestartIfAlreadyPlaying, const bool bLoopMontage)
{
	...
	const float ReportedSlotWeight =
		DefaultSlotWeight > KINDA_SMALL_NUMBER ? DefaultSlotWeight : (bMontagePlaying ? 1.f : 0.f);
...
{
	if (!bEnableBodyMontages || !SpeakingIdleMontage || !IsValid(TargetSkeletalMesh))
	{
		return;
	}
