 ...
 *
 * Montage setup (assets): DefaultSlot upper-body layered blend on GodfreyBodyAnimInstance; assign only
...
public:
	...
	/** When false, skip all body montage plays (face/ACE still run). Leave off until a custom gesture library is ready. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Godfrey|Performer|Bridge|Montages")
	bool bEnableBodyMontages = false;
